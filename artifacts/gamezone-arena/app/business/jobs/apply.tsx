...
export default function Apply(){
  ...
  const submit=()=>{const months=f.experience===""?null:Number(f.experience);if(!jobId||!f.applicant_name.trim()||!f.phone.trim()||months!==null&&(!Number.isInteger(months)||months<0))return Alert.alert("Check application","Enter a name, valid phone number, and whole-number experience.");m.mutate({type:"apply",value:{job_id:jobId,applicant_name:f.applicant_name,phone:f.phone,email:f.email||null,experience_months:months,skills:f.skills.split(",").map(x=>x.trim()).filter(Boolean),introduction:f.introduction||null,message:f.message||null}},{onSuccess:async application=>{if(resume)try{const blob=await(await fetch(resume.uri)).blob();await uploadApplicationResume((application as any).id,resume.fileName||resume.uri.split("/").pop()||"resume.jpg",blob,resume.mimeType||"image/jpeg")}catch(e){console.error("Resume upload failed",e);Alert.alert("Application sent, resume upload failed","Photo upload failed. Please try again.");r.replace("/business/jobs/applications"as any);return}Alert.alert("Applied","Your application was sent.");r.replace("/business/jobs/applications"as any)},onError:(e:Error)=>{console.error("Application failed",e);Alert.alert("Could not apply",e.message)}})};
