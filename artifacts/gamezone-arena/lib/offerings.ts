...
export type BasketItem = { id:string; offering_id:string; buyer_id:string; status:"active"|"cancelled"; created_at:string; updated_at:string; business_offerings:BusinessOffering };
...
export async function myOfferingBasket(){const{data,error}=await db().from("business_offering_basket").select(`id,offering_id,buyer_id,status,created_at,updated_at,business_offerings(${columns})`).eq("status","active").order("created_at",{ascending:false});fail(error,"select","business_offering_basket");return(data??[]).filter((x:any)=>x.business_offerings) as unknown as BasketItem[]}
