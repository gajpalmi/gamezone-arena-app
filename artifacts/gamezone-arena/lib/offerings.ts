...
export async function adminOfferingQueue() { const [o, r] = await Promise.all([db().from("business_offerings").select(ownerColumns).eq("status", "pending"), db().from("business_offering_reports").select("*").is("resolved_at", null)]); fail(o.error); fail(r.error); return { offerings: (o.data ?? []) as BusinessOffering[], reports: r.data ?? [] }; }
